...
const CreateNft = () => {
  ...
  // handle `name`, `description`, and `image`
  ...
  function handleAttributeChange(e: ChangeEvent<HTMLInputElement>) {
    const { name, value } = e.target
  ...
  async function handleImage(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const { url }: UploadFileResponse = res.data
      setNftMeta({
        ...nftMeta,
        image: url
      })
  ...
  async function uploadMetadata() {
    try {
      const res = await toast.promise(axios.post('/api/uploadJson', nftMeta), {
        success: 'upload meta success',
        error: 'upload meta failed',
        pending: 'uploading meta'
      })
      const { URI }: UploadJsonResponse = res.data
      setNftURI(URI)
    } catch (error) {
      // nothing to do
    }
              ...
              <div className="shadow sm:rounded-md sm:overflow-hidden">
                ...
                {nftURI && (
                  <div className="mb-4 p-4">
                    ...
                    <div>
                      <a href={nftURI} className="underline text-indigo-600">
                        {nftURI}
                      </a>
                    </div>
                  </div>
